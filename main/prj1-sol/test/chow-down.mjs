...
describe('chowDown', function() {
  ...
  //new menu case
  it ('must find menu irrespective of case', function () {
    const results = chowDown.menu('4.68','BreaDs');
    assert(results.length > 0);
  });

  //new menu case
  it ('must return a NOT_FOUND error for a non-existent menu id or category', function () {
    const results = chowDown.menu('444','BreaDs');
