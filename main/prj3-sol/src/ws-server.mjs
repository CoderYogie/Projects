...
function setupRoutes(app) {
  ...
  app.post(`${base}/orders?:eateryId`, newOrder(app));
...
function newOrder(app) {
  ...
  return (async function(req, res) {
    try {
      ...
      const eateries = await app.locals.dao.getEatery(eateryId);
      if(eateries.errors)throw eateries;
      const result = await app.locals.dao.newOrder(eateryId);
      if (result.errors) throw result;

      const ret = eateryOrder(eateries,result);
      if (ret.errors) throw ret;

      const data = {...ret,links: [ selfLink(req), eateryLink(req,eateryId), orderLink(req,result.id) ]}
...
function getOrder(app) {
  ...
  return (async function(req, res) {
    try {
      ...
      const result = await app.locals.dao.getOrder(eateryId);
      if (result.errors) throw result;
      const eateries = await app.locals.dao.getEatery(result.eateryId)
      const ret = eateryOrder(eateries,result)
      if(ret.errors)throw ret
      const data = { links: [ selfLink(req),eateryLink(req,result.eateryId) ], ...result };
      res.json(data);
    }
    catch(err) {
      //console.log(err); //uncomment during devel, especially for running tests
...
function updateOrder(app) {
  ...
  return (async function(req, res) {
    try {
      const errors = [];
      const orderId = req.params.orderId;
      const { itemId, nItems } = req.query;
      if(itemId===undefined){
        const msg = `no itemId in update for order \"${orderId}\"`;
	      throw { errors: [ new AppError(msg, { code: 'BAD_REQ'}) ] };
      }else if(nItems===undefined){
        const msg = `bad nItems undefined in update for order \"${orderId}\"`;
	      throw { errors: [ new AppError(msg, { code: 'BAD_REQ'}) ] };
      }
      
      const result = await app.locals.dao.editOrder(orderId,itemId,parseInt(nItems));
      ...
      const data = {...ret, links: [ OrderLinkValue, eateryLink(req,result.eateryId)]}
      res.json(data);
    }
    catch(err) {
      //console.log(err); //uncomment during devel, especially for running tests
...
//logic for getting items without using eatery-order class
function getItems(result,list){
  let items = [];
  let total = 0;
      for(const [key,value] of Object.entries(result.items)){
        const flatMenu = list.flatMenu[key];
        flatMenu['quantity']=parseInt(value);
        flatMenu['quantityPrice']=eval(value*flatMenu['price']);
        items.push(flatMenu);
        total+=flatMenu['quantityPrice'];
      }
  return {items , total};    
}
